...
class ParticipantTest {
    ...
    void extractParticipant() {
        ...
        EventDto eventDto = new EventDto().deserializeEvent(riobm);
        Optional<Participant> participant = new Participant().extractParticipant(eventDto.getData().getBlobUrl());
        ...
        if (participant.isPresent()) {
            ...
            assertEquals("https://storageaccountfordatapoc.blob.core.windows.net/05000005/ArqsAguardando/ARQ0001_CDB_INCL-SIRC.txt", participant.get().blobUrl);
        }

        eventDto = new EventDto().deserializeEvent(pr90302bm);
        participant = new Participant().extractParticipant(eventDto.getData().getBlobUrl());
        ...
        if (participant.isPresent()) {
            ...
            assertEquals("https://storageaccountfordatapoc.blob.core.windows.net/03483002/ArqsAguardando/ARQ0001_RDB_SEMC-SIRC.txt", participant.get().blobUrl);
        }

        eventDto = new EventDto().deserializeEvent(retorno);
        participant = new Participant().extractParticipant(eventDto.getData().getBlobUrl());

        assertEquals(Optional.empty(), participant);
    }
}
